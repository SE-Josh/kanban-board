        ...
        <div className="flex items-center gap-[0.5px] me-1">
          {isEditing ? (
            ...
            <button
              ...
              title="編輯"
            >
              <FaEdit className="text-base-content text-lg opacity-50" />
            </button>
          )}
          {statusId === "done" ? (
            <button
              ...
              title="封存"
            >
              <FaBoxArchive className="text-info text-lg scale-95 opacity-65" />
            </button>
          ) : (
            <button
              className="btn btn-sm btn-circle btn-ghost"
              ...
              title="刪除"
            >
              <MdCancel className="text-error text-lg opacity-50" />
            </button>
          )}
        </div>
      </div>
...
export default function KanbanBoard() {
  ...
  const isExportBundleV1 = (data: any): data is ExportBundleV1 => {
    return (
      ...
      data.labels?.version === 1 &&
      Array.isArray(data.labels?.items)
    );
  };
    ...
    reader.onload = () => {
      try {
        ...
        if (!isExportBundleV1(json)) {
          toast.error("檔案格式或版本不支援（需要 version: 1）");
          return;
        }
        // 套用狀態
    ...
    <div className="mx-auto w-full">
      <div className="mb-5 flex flex-column justify-between items-center">
        ...
        <div className="flex justify-end gap-1">
          ...
          >
            <BiReset className="scale-130" />
            重置
          </button>

          <button
            className="btn btn-soft btn-secondary rounded gap-2"
            onClick={handleExport}
          >
            <FaFileExport className="scale-110" /> 匯出
          </button>

          <button
            className="btn btn-soft btn-primary rounded gap-2"
          ...
          <button
            ...
            onClick={() => {
              copyDailyReport();
              toast.success("日報已複製到剪貼簿！");
            }}
          >
            <BsStars className="scale-140" />
            生成
          </button>
          <button
            ...
            >
              ...
              {editLabelIndex === index ? (
                ...
                <>
                  ...
                  >
                    <FaEdit className="text-base-content opacity-50" />
                  </button>
                </>
              )}
            </li>
